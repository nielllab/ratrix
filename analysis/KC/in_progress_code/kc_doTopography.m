...
for f= 1:length(use) ;
    ...
    if allxshift(f)>-100
        ...
        % KC MADE CHANGES HERE 
        global stdMap
        stdMap = shiftImageRotate(merge{f},allxshift(f)+x0,allyshift(f)+y0,allthetashift(f),allzoom(f),sz);
        mm = stdMap;
        ...
        figure
        subplot(nx,ny,2)
        imshow(mm);hold on; plot(ypts,xpts,'w.','Markersize',2)
