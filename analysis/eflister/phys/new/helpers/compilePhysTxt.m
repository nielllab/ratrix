function compilePhysTxt(analysisDir,rec)
...
for fileNum=1:length(rec)
    ...
    fileGood = false;
    if any(strcmp(contents.mat,compiledFile)) && ~forceStimRecompile && ~forcePhysRecompile
        load(fullfile(fileDir,compiledFile));
    ...
    if ~fileGood
        [phys physTms]=checkForCompiledMat(fileDir,'phys',h,forcePhysRecompile,contents,targetBinsPerSec);
        
        [stim stimTms first(fileNum) step(fileNum)]=checkForCompiledMat(fileDir,'stim',h,forceStimRecompile,contents,[]);
        
        pFile=sprintf('pulse.%s.txt',h);
        origPulses=load(fullfile(fileDir,pFile));
        
        C=doScan(fullfile(fileDir,pFile),'%% %[^%] %%',2);
