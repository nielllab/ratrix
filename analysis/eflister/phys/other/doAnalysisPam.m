...
figure(1)
for stimNum=2:3
    subplot(3,2,stimNum-1)
    for specialNum=order
        if colors(specialNum)~='y' && colors(specialNum)~='b'
    ...
    subplot(3,2,stimNum+1)
    psth=sum((full(repeatSpikes{stimNum}))')/size(repeatSpikes{stimNum},2);
    plot(psth')
    title('PSTH')
    ylabel('P(spike)')
    xlabel('ms')
    
    subplot(3,2,stimNum+3)    
    colorPokes=[repeatPokes{stimNum} ones(size(repeatPokes{stimNum},1),1) uniquePokes{stimNum}]';
    spy(colorPokes,'k',1);
    hold on
    spy(colorPokes==32,'r',10);
    spy(colorPokes==64,'g',10);
    spy(colorPokes==128,'b',10);
    axis fill
    ...
    for silenceNum=1:length(msSilenceRequired)
        ...
        names{silenceNum}=sprintf('%d ms (%d)',msSilenceRequired(silenceNum),size(triggers(silences>msSilenceRequired(silenceNum),:),1));
    end
    ...
    sta(length(msSilenceRequired)+1,:)=mean(bstTrigs)+-1*(length(msSilenceRequired)+1)*rng;
    names{length(msSilenceRequired)+1}=sprintf('burst (%d)',numBstTrigs);
    ...
    hold on
    plot(-preTrig:postTrig,repmat(-1*rng*[0:size(sta,2)],size(sta,1),1),'k')
    xlabel('ms')
    set(gca,'YTick',[])

    title('STA (preceding silence required)')
    ...
    figure(10)
    h2=plot(pTrigs(:,1),pTrigs(:,2),'r.','MarkerSize',5);
    hold on
    h1=plot(pStims(:,1),pStims(:,2),'k.','MarkerSize',5);
    title('projections')    
