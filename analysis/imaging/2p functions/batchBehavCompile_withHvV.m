...
legend(condLabel{1:4}); ylabel('performance'); title(['performance all groups'])     
%response time
  figure
barweb(groupRespTime(:)', groupRespTimeSTD(:)',0.8); ylim([0 3.05]); set(gca,'Ytick',0:1:3.0);
legend(condLabel{1:4}); ylabel('response time (sec)'); title(['response time all groups'])
%stop tim
  figure
barweb(groupStopTime(:)', groupStopTimeSTD(:)',0.8); ylim([0 5.05]); set(gca,'Ytick',0:1:5.0);
legend(condLabel{1:4}); ylabel('stop time (sec)'); title(['stop time all groups'])
    ...
    for t = 1:4
       ...
       n = sum(clust==i & sess==ss)/sum(sess==ss & centered');
        d =nanmean(invariantAll3x(clust==i & sess==ss ,:),1);
          ...
          %diff from behavior
          if t==1
              behavDiff(ss,i,:)=squeeze(SessClustData(ss,t,i,2:30));
              passDiff(ss,i,:) = squeeze(SessClust3xData(ss,t,i,:));
          %flip for supressed
          if i==3
              behavDiff(ss,i,:)=-behavDiff(ss,i,:);
         ...
         DiffAP(ss,i,:) = behavDiff(ss,i,:)-passDiff(ss,i,:);
         DiffAPMod(ss,i,:) = (behavDiff(ss,i,:)-passDiff(ss,i,:))./(behavDiff(ss,i,:)+passDiff(ss,i,:));
...
baselinePass = nanmean(orientInvariant3x(clust==c & allCond==j,baseline,1),2);
...
behavPeak = behavPeak -beselineBehav;
   ...
   figure;
   scatter(behavP{c},passP{c}); title(clustLabel(c)); xlabel('behav peak resp'); ylabel('passive peak resp');
   ...
   scatter(behavP{3},passP{3},'r');legend(clustLabel{1:3});
...
behavPeak = behavPeak -beselineBehav;
   ...
   figure;
   scatter(behavP{j,c},passP{j,c}); title([clustLabel(c) condLabel(j)]); xlabel('behav peak resp'); ylabel('passive peak resp');
   end
     figure;
   ...
   scatter(behavP{j,3},passP{j,3},'k');legend(clustLabel{1:4});
   end
...
for c=1:3
   ...
   scatter(behavP{3,c},passP{3,c},'r');
scatter(behavP{3,c},passP{3,c},'k');legend(condLabel{1:4});
end
   
%%%  difference active passive avg by cell
figure
barweb(squeeze(MeanTaskDiff(:,1:3)'), squeeze(STDTaskDiff(:,1:3)')); ylim([-0.055 0.15]); set(gca,'Ytick',-0.05:0.025:0.1);
legend(condLabel{1:4}); ylabel('response diff'); title(['active vs passive - all cells ']);
%%%%modulation by cell
figure
barweb(squeeze(MeanTaskMod(:,1:3)'), squeeze(STDTaskMod(:,1:3)')); ylim([-0.5 0.55]); set(gca,'Ytick',-0.5:0.25:0.85);
...
figure;
    subplot(2,2,1)
    [n b] = hist((TaskMod{j,1})); bar(b,n/sum(n));title([condLabel{j} 'taskmod transient']); ylim([0 0.3]); xlabel('taskmod'); ylabel('fraction');
    subplot(2,2,2)
    [n b] = hist((TaskMod{j,2})); bar(b,n/sum(n));title([condLabel{j} 'taskmod sustained']); ylim([0 0.3]); xlabel('taskmod'); ylabel('fraction');
    subplot(2,2,3)
    [n b] = hist((TaskMod{j,3})); bar(b,n/sum(n));title([condLabel{j} 'taskmod supressed']); ylim([0 0.3]); xlabel('taskmod'); ylabel('fraction');
end ;  
  %%%distribution of task diff  
for j=1:4
figure;
    subplot(2,2,1)
    [n b] = hist((TaskDiff{j,1})); bar(b,n/sum(n));title([condLabel{j} 'taskmod transient']); ylim([0 0.75]); xlim([-0.5 .5]); xlabel('task diff'); ylabel('fraction');
    subplot(2,2,2)
    [n b] = hist((TaskDiff{j,2})); bar(b,n/sum(n));title([condLabel{j} 'taskmod sustained']); ylim([0 0.75]); xlim([-.5 .5]);xlabel('task diff'); ylabel('fraction');
    subplot(2,2,3)
    [n b] = hist((TaskDiff{j,3})); bar(b,n/sum(n));title([condLabel{j} 'taskmod supressed']); ylim([0 0.75]); xlim([-.5 .5]); xlabel('task diff'); ylabel('fraction');
 end  ;
...
%%%%(includes all 4 training conditions) weighted
 ...
 %%%%timecorse figs of each trining codition for cluster for each stim
%%%%(includes only Naive Rand and GTS) not weighted

figure; set(gcf,'Name','behav unweighted')
for c = 1:3
    subplot(2,2,c)
    plot(squeeze(NWClustSessData(c,1,1:4,:))'); xlim([ 1 30]); %ylim([-0.15 0.22]);
end; legend(condLabel); 
...
for c = 1:3
    subplot(2,2,c)
    plot(squeeze(NWclust3xSess(c,1,1:4,:))'); xlim([ 1 30]);%ylim([-0.15 0.22])
end; legend(condLabel);
...
for c = 1:3
    subplot(2,2,c)
    plot(squeeze(NWclust3xSess(c,2,1:4,:))'); xlim([ 1 30]); ylim([-0.15 0.22])
end; legend(condLabel);
...
% legend(condLabel{1:3}); ylabel('avg response'); title('passive 3x - oblique')
% 
%%%%%%%active vs passive by task condition
figure
barweb(squeeze(respSess(:,1,1,[1 2])), squeeze(respErrSess(:,1,1,[1 3]))); %ylim([-0.025 0.075]); set(gca,'Ytick',-0.025:0.025:0.075);
legend('task','passive 3x'); ylabel('avg response'); title('active vs passive naive')

figure
barweb(squeeze(respSess(:,1,3,[1 2])), squeeze(respErrSess(:,1,3,[1 3]))); %ylim([-0.025 0.075]); set(gca,'Ytick',-0.025:0.025:0.075);
legend('task','passive 3x'); ylabel('avg response'); title('active vs passive gts');
...
figure
barweb(squeeze(respSess(:,1,4,[1 2])), squeeze(respErrSess(:,1,4,[1 3]))); %ylim([-0.025 0.075]); set(gca,'Ytick',-0.025:0.025:0.075);
legend('task','passive 3x'); ylabel('avg response'); title('active vs passive hvv');
...
for i=1:3
% figure
% barweb(squeeze(respSess(i,1,[1:4],1)), squeeze(respErrSess(i,1,[1:4],1))); %ylim([-0.025 0.085]); set(gca,'Ytick',-0.025:0.025:0.085);
% legend(condLabel{1:4}); ylabel('weighted response'); title(['behavior - preferred cluster' num2str(i)])
% 
% figure
% barweb(squeeze(respSess(i,1,[1:4],2)), squeeze(respErrSess(i,1,[1:4],2))); %ylim([-0.02 0.085]); set(gca,'Ytick',-0.01:0.01:0.08);
% legend(condLabel{1:4}); ylabel('weighted response'); title(['passive 3x - cardinal cluster' num2str(i)])
% 
% figure
% barweb(squeeze(respSess(i,2,[1:4],2)), squeeze(respErrSess(i,2,[1:4],2))); %ylim([-0.02 0.085]); set(gca,'Ytick',-0.01:0.01:0.08);
% legend(condLabel{1:4}); ylabel('weighted response'); title(['passive 3x - oblique cluster' num2str(i)])
% 
% figure
% barweb(squeeze(respSess(i,1,[1:4],3)), squeeze(respErrSess(i,1,[1:4],3))); %ylim([-0.02 0.085]); set(gca,'Ytick',-0.01:0.01:0.08);
% legend(condLabel{1:4}); ylabel('weighted response'); title(['passive 2sf - low sf cluster' num2str(i)])
% 
% figure
% barweb(squeeze(respSess(i,2,[1:4],3)), squeeze(respErrSess(i,2,[1:4],3))); %ylim([-0.02 0.085]); set(gca,'Ytick',-0.01:0.01:0.08);
...
figure
barweb(squeeze(APDiffResp(:,1:4)), squeeze(APDiffRespErr(:,1:4))); ylim([-0.025 0.15]); set(gca,'Ytick',-0.025:0.025:0.1);
legend(condLabel{1:4}); ylabel('response diff'); title(['active vs passive ']);
%%%%modulation by session
figure
barweb(squeeze(APDiffRespMod(:,1:4)), squeeze(APDiffRespModErr(:,1:4))); ylim([0 0.85]); set(gca,'Ytick',0:0.25:0.85);
...
%%%%%all clusters and groups
 figure;
barweb(spontModSessGroup(1:4,:)',spontModSessGroupErr(1:4,:)',0.8); ylim([-0.05 .2]);set(gca,'Ytick',0:0.05:0.2);
legend(condLabel{1:4}); title(['running modulation (spont)']); ylabel('mean absolute modulation'); 
   
 figure;
barweb(evModSessGroup(1:4,:)',evModSessGroupErr(1:4,:)',0.8); ylim([-0.05 .2]);set(gca,'Ytick',0:0.05:0.2);
legend(condLabel{1:4}); title(['running modulation (evoked)']); ylabel('mean absolute modulation'); 
...
osifigSS(ss) = figure;
  ...
  for i = 1:max(clust)
   ...
   sprintf('cluster %d trans vert %0.3f sust %0.3f',i,mean(transResp(:,1)),mean(sustResp(:,1)))
    ...
    if i ==2 %for transient cluster %range = 13:14
        peak = max(transResp,[],2);
...
figure
barweb(osiSessGroup(1:4,:)', osiSessGroupErr(1:4,:)',0.8); ylim([-0.05 1.05]); set(gca,'Ytick',0:0.25:1.0);
...
% %(not by session)  using this again!!
for i=1
 clear transOSI sustOSI clustOSI osi osi_err
 for j = 6  %not for each training condition, for all cells only
     condLabel{6}='all cells'
    osifig(j) = figure;
  for i = 1:max(clust)
clear transResp sustResp 
    ThisData{j} = allData(sortClust==i,:);  %took out &sessCond==j to do all cells
...
set(gca,'FontSize',16)
 ...
 figure
bar(osi); hold on; errorbar(1:3,osi,osi_err,'k.','markersize',8,'Linewidth',2);title(['OSI - behavior']); ylabel('osi'); xlabel('cluster'); ylim([0 1])
set(gca,'FontSize',16)
...
legend(clustLabel{1:4}); ylabel('circular varience (OSI)'); title([condLabel{j} ' Cluster OSI 3x passive'])  
close
end;
...
%%%%all groups all clusters
 figure
barweb(circVarGroupSess(1:4,:)', circVarGroupSessErr(1:4,:)',0.8); ylim([-0.05 1.05]); set(gca,'Ytick',0:0.25:1.0);
...
%%%by training condition for each cluster (including inactive)
          figure
barweb(clustfract(1:4,:)', clustfractErr(1:4,:)', 0.8); ylim([0 0.65]); set(gca,'Ytick',0:0.1:0.75);
