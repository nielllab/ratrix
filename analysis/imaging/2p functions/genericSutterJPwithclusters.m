...
imagesc((selecttrace(perm,:)),[-0.5 0.5]); axis xy ; xlabel('selected traces based on dF'); colormap jet ;   %%% show sorted data
hold on;
totalT = size(dF,2);
...
for clust = 1:nclust
    figure
    subplot(1,2,1);
imagesc(stdImg,[0 500]); hold on;colormap gray;freezeColors;
title(sprintf('cluster %d',clust));

    plot(yselected(c==clust),xselected(c==clust),'go')%%'Color',colors(c));
    subplot(1,2,2); 
