...
%         sprintf('%d cells with good topo under cutoff',length(allgoodTopo))
    ...
    %     spikesOut = spikesOut(goodTopo,:,:);
        ...
        dftuningall = zeros(size(dFout,1),size(dFout,2),length(sfrange),length(tfrange),length(dirrange),2);
        sptuningall = zeros(size(spikesOut,1),size(spikesOut,2),length(sfrange),length(tfrange),length(dirrange),2);
        for h = 1:size(dFout,1)
            for i = 1:length(sfrange)
                for j = 1:length(tfrange)
                    for k = 1:length(dirrange)
                        for l = 1:2
                            dftuningall(h,1:size(dFout2,2),i,j,k,l) = nanmean(dFout2(h,:,find(sf==sfrange(i)&tf==tfrange(j)&theta==dirrange(k)&running==(l-1))),3);
                            sptuningall(h,1:size(spikesOut2,2),i,j,k,l) = nanmean(spikesOut2(h,:,find(sf==sfrange(i)&tf==tfrange(j)&theta==dirrange(k)&running==(l-1))),3);
                        end
                    end
                end
            end
        ...
        [bestsftfresp(:,1) bestsftf(:,1)] = max(squeeze(nanmean(nanmean(nanmean(nanmean(dftuningall(:,dfWindow,:,:,:,1),2),4),5),6)),[],2); %best sf
        [bestsftfresp(:,2) bestsftf(:,2)] = max(squeeze(nanmean(nanmean(nanmean(nanmean(dftuningall(:,dfWindow,:,:,:,1),2),3),5),6)),[],2); %best tf
        dfori = nan(size(dFout2,1),12);
        for i=1:size(dFout2,1)
            dfori(i,:) = squeeze(nanmean(dftuningall(i,dfWindow,bestsftf(i,1),bestsftf(i,2),:,1),2));
        ...
        allCellsind = find(allCells==1);allCellsind=allCellsind(allCellsind<cellCutoff);
        
        respthreshlist = [0.025, 0.05, 0.1, 0.2, 0.5, 1.0];
        figure
        for i = 1:length(respthreshlist)
            respcells = find(max(dfori,[],2)>=respthreshlist(i)); respcells=respcells(respcells<cellCutoff);
    %         sprintf('%d responsive cells under cutoff',length(respcells))  
            %%%plot responsive cells on screen w/non-responsive
            subplot(2,3,i)
            hold on
            plot(rf(allCellsind,2),rf(allCellsind,1),'.','color',[0.5 0.5 0.5],'MarkerSize',10); %%% the rfAmp criterion wasn't being applied here
            plot(rf(respcells,2),rf(respcells,1),'b.','MarkerSize',10);
            circle(ycent,xcent,centrad/dpix)
            axis equal;
            axis([0 72 0 128]);
            set(gca,'xticklabel','')
            xlabel(sprintf('%.1f dfof',respthreshlist(i)*100))
        end
        ...
        respcells = find(max(dfori,[],2)>=respthreshlist(3)); respcells=respcells(respcells<cellCutoff);
        ...
        for i=1:length(respcells)
            ...
            hold on
%             resp = squeeze(dftuningall(respcells(i),:,bestdf(respcells(i),1),bestdf(respcells(i),2),ind,1));
            [respmax respmaxi] = max(dfori(respcells(i),:));
            resp = squeeze(dftuningall(respcells(i),:,bestsftf(respcells(i),1),bestsftf(respcells(i),2),respmaxi,1));
            plot(timepts,resp,'k')
%             plot(timepts,respmaxs,'r')
        ...
        save(filename,'dftuning','sptuning','osi','dsi','prefori','prefdir','bestsftf','preftheta','prefthetaQuad','respcells')
