%%% analyze behavior performance with laser stimulation
...
%%% read in behav data
[f p]= uigetfile('*.mat');
load(fullfile(p,f),'trialRecords');
...
legend({'off','on'})
ylabel('fraction left'); set(gca,'Xtick',[1 2]); set(gca,'Xticklabel',{'top','bottom'});
...
ylabel('median resp time');set(gca,'Xtick',[1 2]); set(gca,'XtickLabel',{'off','on'}); ylim([ 0 1]);
