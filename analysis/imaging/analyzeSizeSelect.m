    ...
    for j= 1:length(radiusRange)
        for k = 1:length(sfrange)
            for l=1:length(tfrange)
                cond = cond+1;
                avgtrialdata(:,:,cond) = squeeze(median(trialdata(:,:,find(xpos==xrange(i)&radius==radiusRange(j)&sf==sfrange(k)&tf==tfrange(l))),3));%  length(find(xpos==xrange(i)&ypos==yrange(j)&sf==sfrange(k)&tf==tfrange(l)))
                avgtrialcourse(i,j,k,l,:) = squeeze(median(trialcourse(find(xpos==xrange(i)&radius==radiusRange(j)&sf==sfrange(k)&tf==tfrange(l)),:),1));
                ...
                meanspd(i,j,k,l) = squeeze(mean(trialspeed(find(xpos==xrange(i)&radius==radiusRange(j)&sf==sfrange(k)&tf==tfrange(l)))>500));
                trialcycavg(:,:,:,i,j,k,l) = squeeze(mean(trialcyc(:,:,:,find(xpos==xrange(i)&radius==radiusRange(j)&sf==sfrange(k)&tf==tfrange(l))),4));
            end
        end
    end
end
...
%%plot cycle averages for the different radii
figure
for i = 1:length(xrange)
    subplot(1,length(xrange),i)
    hold on
