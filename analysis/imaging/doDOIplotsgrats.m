...
%         plot(squeeze(allmnfit(x(area)+(-2:2),y(area),1:4,:).*squeeze((amp(x(i),y(i),:))%%% un-normalize it
    title(sprintf(cell2mat(areanames(area))));
    ylim([0 max(max(max(allmnfit(:,:,1:3))))])
end
...
subplot(2,4,1)
plot(0,squeeze(allmnfit(1,1,1,conds)))
title('Time course','position',[0.5 0.5],'color','r','fontsize',20)
axis([0 1 0 1])
axis off
legend(datafiles(conds),'location','south')
for area = 1:7
...
for area = 1:7
    ...
    plot(0.1:0.1:(0.1*length(d)),(circshift(d,5)));
    axis([0.1 1.5 0 1])
end
...
% for area = 1:7
%     subplot(2,4,area+1)
% plot(squeeze(mean(mean(allshiftData(x(area)+range, y(area)+range, 4:5,conds),2),1)))
% %         plot(squeeze(allmnfit(x(area)+(-2:2),y(area),1:4,:).*squeeze((amp(x(i),y(i),:))%%% un-normalize it
%     title(sprintf(cell2mat(areanames(area))));
%     ylim([0 1])
% end
...
% for area = 1:7
%     subplot(2,4,area+1)
%     plot(squeeze(mean(mean(allshiftData(x(area)+range, y(area)+range, 6:7,conds),2),1)))
% %         plot(squeeze(allmnfit(x(area)+(-2:2),y(area),1:4,:).*squeeze((amp(x(i),y(i),:))%%% un-normalize it
%     title(sprintf(cell2mat(areanames(area))));
%     ylim([0 1])
% end
