function r = sbxalign_tif_nonrigid(fname,idx)
...
if(length(idx)==1)
    ...
    A = double(imread(fname,idx(1))); % read the frame
...
elseif (length(idx)==2)
    A = double(imread(fname,idx(1))); % read the frame
    B = double(imread(fname,idx(2))); % read the frame
...
%     r.m{4} = zeros(size(A)); % 4th moment
    ...
    r0 = sbxalign_tif_nonrigid(fname,idx0); % align each group
    r1 = sbxalign_tif_nonrigid(fname,idx1);
