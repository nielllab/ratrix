...
for f = 1:length(use)
    ...
    filename = sprintf('%s_%s_%s_%s_patchonpatch.mat',files(use(f)).expt,files(use(f)).subj,files(use(f)).inject);
...
%                 end
%             catch
                load(fullfile(outpathname,sprintf('%s_%s_%s_%s_patchonpatch.mat',files(use(f)).expt,files(use(f)).subj,files(use(f)).inject)),...
                    'x','y','dist')
%                 load(fullfile(altpathname,filename),'x','y','dist')
                figure;
                colormap jet
