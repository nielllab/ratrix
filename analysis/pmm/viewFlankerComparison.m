...
if ~exist('addNames','var') || isempty(addNames)
    ...
    if islogical(addNames) && addNames
...
if numStats==1
    ...
    disp('since only one stat, making a square marix of plots')
    ...
    for j=1:numStats
             switch statTypes{j}
                ...
                case {'biasMCMC', 'criterionMCMC','dprimeMCMC','dpr','RT'}
                    ...
                    stdPop=std(popStats);
%                     delta(i,j)=log(...  %beware: if negative dprime, log ratio gets imaginary numbers
%                         mean(params.stats(:,cMatrix{i,1},statInd))./...%log ratio
%                         mean(params.stats(:,cMatrix{i,2},statInd)));%works on the average of the statistics, not a weighed average
...
for j=1:numStats
    ...
    for i=whichComparison
        if doFigAndSub
            if squareStats
                %factorable code if analysis and plotting are split
                subplot(II, JJ,i);
            else
                subplot(numComparison, numStats,(i-1)*numStats+j);
            end
        ...
        if i<=size(CI,1) %only for some comparisons
            ...
            plot([CI(i,j,1) CI(i,j,2)], repmat(populationYVal,1,2), 'color', color,'lineWidth',4);
            plot(delta(i,j), populationYVal, 'o', 'markerSize', 7, 'color', color);
        end

        axis([minX maxX (maxBinCount+eps)*[-1 3.2]]) %eps fixes error if maxBinCount=0
...
for j=1:numStats
    if doFigAndSub
        if squareStats
            ...
            subplot(numComparison, numStats, (numComparison-1)*numStats+j);
        end
    end
...
for i=1:numComparison
    if doFigAndSub
        if squareStats
            ...
            subplot(numComparison, numStats, (i-1)*numStats+1);
        end
    end
