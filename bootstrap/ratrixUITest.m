...
rackNum=1;
server_uin=getServerUINFromIP; %this needs to be determined by a function getServerUINFromIP (based on what this machine's IP is)
if isempty(server_uin)
    server_uin=3;
end
...
%     hts{i}.name
    assignments=getAssignmentsForServer(conn,server_uin,hts{i}.name);
    coaches = getCoachAssignmentsForServer(conn,server_uin,hts{i}.name);
...
rrs = getRacksAndRoomsFromServerUIN(conn,server_uin);
