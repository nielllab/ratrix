...
switch nargin
    case 0
        ...
        s = class(s,'CNMafc',stimManager());
    case 1
        % if single argument of this class type, return it
        if (isa(varargin{1},'CNMafc'))
            s = varargin{1};
        else
            error('Input argument is not a CNMafc object')
        end
    case 6
        ...
        switch s.soundType
            ...
            otherwise
                error('CNMafc: soundType not recognized')
        end
        
        s = class(s,'CNMafc',stimManager(varargin{3},varargin{4},varargin{5},varargin{6}));
