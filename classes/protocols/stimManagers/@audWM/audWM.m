...
switch nargin
    case 0
        ...
        s = class(s,'audWM',stimManager());
    case 1
        % if single argument of this class type, return it
        if (isa(varargin{1},'audWM'))
            s = varargin{1};
        else
            error('Input argument is not a audWM object')
        end
    case 6
        ...
        switch s.soundType
            ...
            case {'tone', 'CNMToneTrain','wmToneWN','warblestackWav'}
                %if soundParams.freqs > 0
                    s.freqs=soundParams.freqs;
            ...
            otherwise
                error('audWM: soundType not recognized')
        end
        
        s = class(s,'audWM',stimManager(varargin{3},varargin{4},varargin{5},varargin{6}));
