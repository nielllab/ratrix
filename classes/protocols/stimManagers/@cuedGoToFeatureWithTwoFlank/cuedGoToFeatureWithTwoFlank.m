function s=cuedGoToFeatureWithTwoFlank(varargin)
...
% if no input arguments, create a default object
    ...
    s.pixPerCycs = [];
    s.targetOrientations = [];   %the first one is what you go to, the second one is what you go away from
...
%     s.flankerStim =zeros(2,2,1);
    ...
    s = class(s,'cuedGoToFeatureWithTwoFlank',stimManager());   
...
% if single argument of this class type, return it
    if (isa(varargin{1},'cuedGoToFeatureWithTwoFlank'))
        ...
        error('Input argument is not an cuedGoToFeatureWithTwoFlank object')
    end
case 24
