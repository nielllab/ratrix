...
details.pctCorrectionTrials=0; % need to change this to be passed in from trial manager
...
%     %orientation of the feature that is indicative of where to go
     details.goToOrient=stimulus.targetOrientations(Randi(a));
...
% VERY OLD EXAMPLE params = [repmat([stimulus.radius details.pixPerCyc],numGabors,1) details.phases details.orientations repmat([stimulus.contrast stimulus.thresh],numGabors,1) details.xPosPcts repmat([stimulus.yPosPct],numGabors,1)];
    ...
    ([ stimulus.targetYPosPct       stimulus.targetYPosPct        1/2-responseIsLeft*ecc    1/2-responseIsLeft*ecc;...    %target
       stimulus.targetYPosPct       stimulus.targetYPosPct        1/2+responseIsLeft*ecc    1/2+responseIsLeft*ecc;...    %distractor
      ...
      stim(pos(i,1):pos(i,2),pos(i,3):pos(i,4),1)=stim(pos(i,1):pos(i,2),pos(i,3):pos(i,4),1)+(stimulus.flankerStim(:,:,orientInd)-stimulus.mean)*details.flankerContrast;
      i=i+1;
      stim(pos(i,1):pos(i,2),pos(i,3):pos(i,4),1)=stim(pos(i,1):pos(i,2),pos(i,3):pos(i,4),1)+(stimulus.flankerStim(:,:,orientInd)-stimulus.mean)*details.flankerContrast;
      ...
      %RENDER CUE - side cue not used, only fixation odt
