...
try
    ...
    % timeWindowMs
    timeWindowMs=[500 50]; % parameter [300 50]
    ...
    if plotParamters.showSpikeAnalysis
        figureName = [num2str(min(cumulativedata.cumulativeTrialNumbers)) '-' num2str(max(cumulativedata.cumulativeTrialNumbers))];
        figure(min(cumulativedata.cumulativeTrialNumbers),'Name',figureName) % trialBased is better
