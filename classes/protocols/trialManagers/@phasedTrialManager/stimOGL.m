...
stimOGL(tm,stimSpecs,finalPhase,soundTypes,LUT,scaleFactors,window,ifi,targetOptions,distractorOptions,requestOptions, ...
  ...
  %  stimInd = 1;
	...
	%    frameIndex=0;
        ...
        %        Screen('CopyWindow', window, window);
            ...
            yNewTextPos=yNewTextPos+1.5*normBoundsRect(4);
            msg = sprintf('transition criterion is %d', transitionCriterion{1});
            Screen('DrawText',window,msg,xTextPos,yNewTextPos,100*ones(1,3));
            ...
            if ~isempty(soundTypesInPhase)
                for stInd=1:length(soundTypesInPhase)
                    ...
                    if shouldWePlay(st,portSet, stepsInPhase)
                        tm.soundMgr = playSound(tm.soundMgr,getName(st),getDuration(st)/1000.0,station);
%                         tm.soundMgr = playLoop(tm.soundMgr,getName(st),station,getReps(st));
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Check phase transitions (graduation)
            ...
            % Check port against graduationCriterion - GRADUATION BY RESPONSE
            updatePhase = 0;
