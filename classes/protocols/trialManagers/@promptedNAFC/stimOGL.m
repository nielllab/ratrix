...
% sounds not cached - turned off with if 0,
% playLoop -- all return values set to "junk" (we are supposed to keep them so play is not interupted...?)
% all use of playLoop uses the get function to extract the soundManager
% added responseDetails.isStochastic
% added responseDetails.isHuman (maybe should add to germline stimOGL too!)
% added getPromptLikelihood
% added numBlockedFrames=25, which stops the animal from responding within 25 frames after the first request
...
try
    ...
    didValves=0;
    didManual=0;
    requestFrame=0;  %used for timed frames stimuli and numBlockedFrames
    blockedResponse=0;
    numBlockedFrames=50; %prompt changed
            ...
            if stimStarted
                ...
                if ~trigger
                    if ~isempty(audioStim)
                        % Play audio
                        junk = playLoop(getSoundManager(tm),audioStim,station,1);
                    ...
                    case 'textureCache'
                        ...
                        elseif trigger
                            if isRequesting
                                ...
                                if audioStimPlaying
                                    % Turn off audio
                                    junk = playLoop(getSoundManager(tm),'',station,0);
                                    audioStimPlaying = false;
                                end
                                i=2;
                            end
...
%%            
        ...
        if keyIsDown
            ...
            rewardKeyShortCut=1;
            if rewardKeyShortCut & length(ports)<4
                %map shortcut left center right

                downArrow=0;
                for keyNum=1:length(keys)
                    downArrow= downArrow || any(strcmp(KbName(keys(keyNum)),'down'))
                    any(strcmp(KbName(keys(keyNum)),'down'));
                end
        ...
        if ~paused
            ...
            if isa(tm,'promptedNAFC') && all(~ports) %changedPromptVerion
                if rand<getPromptLikelihood(tm)
                    if ~isempty(requestOptions)
                        requestPort=requestOptions(1)
                    else
                        requestPort=[]; % don't do anything when there is nothing to do!
                    end
                    ports(requestPort)=1; %use the first request option, typically port 2 for 3 ports,  beware the station assumption: is hardware only 2 port!? and software 3 port?  then okay
                ...
                % if using this, maybe move it below to where free drinks does it
                %if rand<getFreeDrinkLikelihood(tm)
                %                     %ports(ceil(rand*length(responseOptions)))=1; %whoops -- bug
                %                     ports(2)=0; %not
                %                     port(responseOptions(ceil(rand*length(responseOptions))))=1; %will still be wrong half the time...
                %                     ports(ceil(rand*getNumPorts(station)))=1;
                %                     potentialStochasticResponse=1; %might not be a viable response option, so don't log didStocasticResponse yet
                %                 else
                %                     potentialStochasticResponse=potentialStochasticRespone; %from above
                %                 end
            ...
            if any(ports(requestOptions))
                ...
                if isempty(audioStim)
                    junk = playLoop(getSoundManager(tm),'keepGoingSound',station,1);
                end
            ...
            if any(ports(responseOptions)) && stimStarted && ~blockedResponse %prompt modified
                done=1;
                response = ports;
                logIt=1;
                stopListening=1;
            ...
            if any(ports) && ~stopListening
                if isempty(audioStim)
                    junk = playLoop(getSoundManager(tm),'trySomethingElseSound',station,1);
                end
            ...
            if logIt
                ...
                responseDetails.isStochastic{attempt}=potentialStochasticResponse; %small chance a real response is labeled as stochastic, but no chance stochastic is dropped
                responseDetails.isHuman{attempt}=didHumanResponse; %if overlap, then real response is labeled as human, but no chance human is dropped
                lookForChange=1;
                logIt=0;
            end
            ...
            if requestFrame~=0 && frameNum-requestFrame<numBlockedFrames  %prompt added
                blockedResponse=1;
            else
                blockedResponse=0;
            end
    ...
    audioStimPlaying=false;
    junk= playLoop(getSoundManager(tm),'',station,0); %changedPromptVerion (others like this too)
