...
if strmatch(class(trialManager), 'nAFC')
    ...
    % waiting for request
    criterion = {[2], [2]};
    stimSpecs{1} = stimSpec(getInterTrialLuminance(stimManager),criterion,'cache',1,[],rewardSizeULorMS,targetPorts,framesUntilTransition,[],0,0);
    % waiting for response
    %reverse frames
%     reverse_stim(:,:,1) = stim(:,:,2);
%     reverse_stim(:,:,2) = stim(:,:,1);
    criterion = {targetPorts, 3, distractorPorts, 4};
    stimSpecs{2} = stimSpec(stim,criterion,type,1,[],rewardSizeULorMS,targetPorts,[],[],scaleFactor,0);
    % correct
    criterion = {[], 5};
    numCorrectFrames=ceil((rewardSizeULorMS/1000)/ifi);
    stimSpecs{3} = stimSpec(getInterTrialLuminance(stimManager),criterion,'cache',1,'reward',rewardSizeULorMS,targetPorts,numCorrectFrames,[],0,0);
    % wrong
    criterion = {[], 5};
    numErrorFrames=ceil((msPenalty/1000)/ifi);
    [errStim errorScale] = errorStim(stimManager,numErrorFrames);
    stimSpecs{4} = stimSpec(errStim,criterion,'cache',1,[],rewardSizeULorMS,targetPorts,numErrorFrames,[],errorScale,0);
    % final
    criterion = {[], 1};
    stimSpecs{5} = stimSpec(getInterTrialLuminance(stimManager),criterion,'cache',1,...
        [],rewardSizeULorMS,targetPorts,1,[],0,1);
elseif strmatch(class(trialManager), 'freeDrinks')
    ...
    stimSpecs{1} = stimSpec(stim, criterion,'loop',1,[],rewardSizeULorMS,targetPorts,[],{stochasticP,1,stochasticP,2,stochasticP,3},scaleFactor,0);
    % correct phase (always correct)
    criterion = {[], 3};
    numCorrectFrames=ceil((rewardSizeULorMS/1000)/ifi);
    stimSpecs{2} = stimSpec(getInterTrialLuminance(stimManager),criterion,'cache',1,'reward',rewardSizeULorMS,targetPorts,numCorrectFrames,[],0,0);
    % final
    criterion = {[], 1};
...
elseif strcmp(class(trialManager), 'autopilot')
    ...
    criterion = {[], 2};
    stimSpecs{1} = stimSpec(stim, criterion,'loop',1,[],rewardSizeULorMS,targetPorts,[],[],scaleFactor,0);
    % final phase
    criterion = {[], 1};
