...
selectAssignmentQuery = ...
    sprintf('SELECT subjects.display_uin, rack_id, station_id, heats.name, researchers.username, experiments.name FROM heat_assignments,subjects,racks,stations,heats,researchers,experiments WHERE heat_assignments.subject_uin=subjects.uin AND heat_assignments.station_uin=stations.uin AND stations.rack_uin=racks.uin AND heat_assignments.heat_uin=heats.uin AND subjects.owner_uin=researchers.uin(+) AND heat_assignments.experiment_uin=experiments.uin AND racks.rack_id=%d AND heats.name=''%s''ORDER BY rack_id,station_id',rack_id,heat_name); 
