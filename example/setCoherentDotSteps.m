...
%fd1 = freeDrinks(msFlushDuration,rewardSizeULorMS,msMinimumPokeDuration,msMinimumClearDuration,sm,msPenalty,msRewardSoundDuration,freeDrinkLikelihood,rewardManager);
fd1 = freeDrinks(msFlushDuration,msMinimumPokeDuration,msMinimumClearDuration,sm,freeDrinkLikelihood,rewardManager);

freeDrinkLikelihood=0;
%fd2 = freeDrinks(msFlushDuration,rewardSizeULorMS,msMinimumPokeDuration,msMinimumClearDuration,sm,msPenalty,msRewardSoundDuration,freeDrinkLikelihood,rewardManager);
fd2 = freeDrinks(msFlushDuration,msMinimumPokeDuration,msMinimumClearDuration,sm,freeDrinkLikelihood,rewardManager);
...
miniDatabasePath = 'C:\acalhoun\miniDatabase'
boxIDs=getBoxIDs(r);

for i=1:length(subjects)
    subjObj = subject(subjects{i}, 'rat', 'long-evans', 'male', '05/10/2005', '01/01/2006', 'unknown', 'wild caught');
    %r=addSubject(r,subjObj,'pmm'); %done by rack already...
    %step = getDatabaseFact(getSubjectFromID(r,char(subjects(i))),'stepNumber')
    step=getMiniDatabaseFact(getSubjectFromID(r,subjects{i}),'stepNumber',miniDatabasePath); %this is if you have to reinit...
    [subjObj r]=setProtocolAndStep(subjObj,p,1,0,1,step,r,'adam rats','pmm');
    %r=putSubjectInBox(r,getID(subjObj),boxIDs(1),'pmm');
    r=setValuesFromMiniDatabase(subjObj,r,miniDatabasePath);         
end