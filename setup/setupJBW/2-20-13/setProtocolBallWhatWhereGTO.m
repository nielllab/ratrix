function r = setProtocolWhatWhereGTO(r,subjIDs)
...
%abstractSF = orientedGabors(pixperdeg./cpd,{distractorOrientations [] targetOrientations},'abstract',mean,radius,contrast,thresh,yPosPct,maxWidth,maxHeight,scaleFactor,interTrialLuminance,[],[],[0.25 0.75]);



%%% abstract orientation (e.g. -pi/4 = go left, pi/4 = go right)
targetOrientations = pi/4;
distractorOrientations = -pi/4;

stim.stim = orientedGabors(pixPerCycs,{distractorOrientations [] targetOrientations},'abstract',mean,radius,contrast,thresh,yPosPct,maxWidth,maxHeight,scaleFactor,interTrialLuminance,[],[],xPosPct);
 ballSM = trail(stim,maxWidth,maxHeight,zoom,interTrialLuminance);
 ballTM = ball(percentCorrectionTrials,sm,noRequest);
 
 ts1 = trainingStep(ballTM, ballSM, repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode); %ball
 
 p=protocol('go to orientation',{ts1});
