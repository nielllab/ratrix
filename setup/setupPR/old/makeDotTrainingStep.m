...
% defaults
if ~exist('trialmanager','var'), % if none specified,
    trialmanager=maketrialmanager; % default nAFC trial manager
end
if ~exist('performancecrit','var'), % if none specified,
    performancecrit=performanceCriterion([0.85, 0.8],int16([200, 500]));
end
if ~exist('scheduler','var'),
    scheduler=noTimeOff(); %minutesPerSession(90,3); %%CHANGED 11/11/08 not sure if coherentdot stim method even listens to this
end
if ~exist('coherence','var'),
    error('Please specify coherence between 0 and 1')
end

dot_size=3; % default size in pixels
speed =1;%speed (pixels/frame)
