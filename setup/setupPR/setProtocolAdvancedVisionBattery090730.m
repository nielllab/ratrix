...
%7/30/09 move to new tag
svnRevision={'svn://132.239.158.177/projects/ratrix/tags/v1.0.2'};
...
imageSizeYoked=false; %%7/30/09 CHANGE!! size varies independently 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% objrec3invar=gotos+ ignore s- [Nike Shuttle]
% exactly as previous but more distractors
imageSelectionMode='normal'; % not deck
imageSize=[.75 1]; % smaller range, uniformly between these sizes
imageSizeYoked=false; %  
imageRotation=[-25 25]; % smaller range of angles
imageRotationYoked=false; % images  size varies independently
imagelevel3invar = images(imdir,ypos_nAFC,background_nAFC,...
    maxWidth,maxHeight,scaleFactor,interTrialLuminance_nAFC, imlist.level4easy,... % NEW image list
    imageSelectionMode,imageSize,imageSizeYoked,imageRotation,imageRotationYoked,drawingMode);
%define the associated training step
objrec3invar = trainingStep(gotostim, imagelevel3invar, graduationCriterion, scheduler,svnRevision); 


%%%PROTOCOLS%%%%%
 
%pAdvTest=   protocol('advanced vision tests', {objrec1 objrec2 objrec2invar});  
pAdvTest090730=   protocol('advanced vision tests', {objrec1 objrec2 objrec2invar objrec3invar});  

%%%%%%%%%%%%
thisIsANewProtocol=1; % typically 1
thisIsANewTrainingStep=1; % typically 0
thisIsANewStepNum=1;  %  typically 1
stepind=1; % 7/30/09 starting point

logPath=fullfile(fileparts(fileparts(getRatrixPath)),'ratrix\setup\setupPR',filesep);
cd(logPath)
fid=fopen('Blindsightlog_PRserver_v1.0.2.txt','a'); % start a new log
fprintf('svn ver %s\n',svnRevision{1});
for i=1:length(subjIDs),
    subjObj = getSubjectFromID(r,subjIDs{i});
    if ismember(subjIDs{i},ValidRats) % define rat ID list at top of this file
        
        [subjObj r]=setProtocolAndStep(subjObj,pAdvTest090730,thisIsANewProtocol,thisIsANewTrainingStep,thisIsANewStepNum,stepNum,r,'Advanced Vision Tests 090730','pr');
        fprintf(fid,'%s finished setting %s to step %d of pAdvTest090730\n', datestr(now),subjIDs{i},stepNum);
