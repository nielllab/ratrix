...
Subject
    ...
    earID     EarID
    gender    Gender
    --exactly one cage and litter, zero or more cohorts
    genotype  Genotype -- compatible w/parents
...
Researcher
    firstName String
    lastName  String --req, unique w/firstname
    email     Email
    phone     Phone
...
Litter
    listOfSubjects
    dateOfBirth    Date
    mother         Subject -- female with earlier DOB
    father         Subject -- male with earlier DOB
    species        Species
    strain         Strain
    supplier       Supplier
...
postPersonFR = do
    ...
    defaultLayout $ case result of
        FormSuccess person -> [whamlet|<p>#{show person}|]
        _                  -> [whamlet|
